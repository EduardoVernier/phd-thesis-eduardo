...
\textit{
The previous two chapters have shown two evaluations of treemapping algorithms that are used for visualizing time-dependent hierarchies. Both evaluations -- the first being done only for datasets obtained from evolving software systems (Chapter~2) and the second being done for a much wider spectrum, and number, of datasets obtained from many application domains (Chapter~3) point out that there is no ideal algorithm that can optimize both spatial quality and stability. As such, there is room to create better treemapping algorithms. In this chapter, we finalize the treemap trach of this thesis by proposing such an algorithm, with a focus on the visualization of time-dependent software hierarchies. Our method, called \emph{Greedy Insertion Treemaps}, is a simple and scalable technique which achieves a good balance between temporal stability and visual quality, scoring better values than
existing techniques, as supported by the software-datasets benchmark introduced in Chapter~2. 
}

\alex{I modified the above since, in this chapter, we benchmarked GIT only against the \emph{software} hierarchies, not the entire benchmark from Chapter~3. I know that GIT was also used in the evaluation in Chapter~3, but, in this chapter, we only refer to software datasets. So I think it is better to position GIT as we did initially, meaning, as a good algo for software evolution data.}
...
\alex{I am modifying the conclusion here, since the old one was from the paper and that didn't account for the `big' comparison from Chapter 3.}
