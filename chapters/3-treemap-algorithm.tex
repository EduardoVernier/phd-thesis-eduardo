...
\manualmark
\markboth{\spacedlowsmallcaps{Treemap Algorithm}}{\spacedlowsmallcaps{Treemap Algorithm}} 
\phantomsection 

\pdfbookmark[0]{Treemap Algorithm}{treemap-algorithm}
...
\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{figures/treemap-algorithm/space-partition.png}
...
\begin{figure}[htbp!]
\centering
\includegraphics[width=.48\textwidth]{figures/treemap-algorithm/git_example.eps}
...
After handling all changes in a new revision of $T$, we rebuild ${\cal T}$ from $LT$, as already explained. As we show next in Sec.~\ref{sec:results}, GIT scores a very good balance of spatial quality \emph{vs} stability.

\begin{figure}[htbp!]
\centering
\includegraphics[width=.45\textwidth]{figures/treemap-algorithm/git_removal.eps}
...
\label{sec:results}
%
We evaluate GIT on the aforementioned datasets, algorithms, and metrics collection from several perspectives, by answering a series of questions. Below, average stability $S$ refers to the average of the $LDC$ and $Q$ metrics introduced in Sec.~\ref{sec:metrics}. All results that we were not able to fit in the paper can be found at our online repository\cite{benchmark}.
