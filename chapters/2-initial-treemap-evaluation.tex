\chapter{Treemap Evaluation for Software Data}
\label{ch:soft-eval}
% \title{Quantitative Comparison of Dynamic Treemaps for Software Evolution Visualization}

\textit{
We start our study of treemapping methods with an evaluation. The ultimate goal (which is realized in Chapter 3) is to be able to reliably and fairly test treemapping algorithms on generalized data, \emph{i.e.}, a large number of datasets from different sources, displaying a variety of traits and dynamics. However, for our initial study, we chose a smaller scope and focus solely on hierachies originated from software repositories. In this chapter, we also introduce an important theme that is repeated many times throughout the thesis: we develop temporal stability metrics that reliably quantify the relationship between data change and visual change.
}
