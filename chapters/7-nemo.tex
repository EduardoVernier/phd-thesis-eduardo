...
Hyperkinetic movement disorders are a class of disorders that are characterised by excessive and involuntary motor movements. These disorders include a range of different  phenotypes such as myoclonus, dystonia, tremor, chorea and tics. Since the clinical presentation and ethiology differs between these disorders, different protocols are required for clinical treatment. Treatments differ e.g., in the choice of additional diagnostic tests, medication prescriptions, and deep brain stimulation targets. It is therefore of utmost importance to ensure accurate phenotypic classification. As hyperkinetic movement disorders have no clear anatomical abnormalities, classification is based on clinical definitions and thus on expert opinion. What makes this increasingly difficult is that that complex and mixed forms of hyperkinectic movement disorders occur in many patients, and many clinical features of these disorders also correspond to clinical features of other movement disorders classes such as ataxia, spasticity, and functional movement disorders. Moreover, there is large inter- and intra-observer classification variability based on clincial definitions by expert clinicians (REF 1-4).

To improve hyperkinetic movement disorder phenotyping, the Next Move in Movement Disorders (NEMO) study was set up at the University Medical Center Groninge, The Netherlands. The aim of this study is to build computer aided classification tools for hyperkinetic movement disorders to assist healthcare professionals in phenotyping this class of disorders. In the NEMO study, a large dataset is being collected from hyperkinectic movement disorder patients and healthy controls while they perform 40 (motor and non-motor) tasks in different settings. During these tasks data is measured using electromyography (EMG), motion sensors (accelerometry), 2D & 3D video, positron emission tomography (PET), and functional magnetic resonance imaging (fMRI). Patients are included in this study based on independent expert panel agreement on their hyperkinetic movement disorder phenotype. 

The current study focuses on the EMG amd accelerometry data of the NEMO study. Accelerometry is applied in clinical practice to support hyperkenitic movement disorder phenotyping or particularly myoclonus, tremor, and dystonia patients. Typically, muscle activation pattenrs, tremor burst duratons, and frequecies are used for such assesments.

>> DESCRIBE ACCELEROMETRY IN MORE DETAIL HERE INCLUDING PROS AND CONS. THIS WILL LEAD TO THE CURRENT GOAL OF VISUALIZING THE DATA WITH DYNAMIC PROJECTION METHODS TO 1) DISCERN PATIENT GROUPS, AND 2) IDENTIFY CLINICAL/MEASURMENT FEATURES THAT HELP DEFINE PATIENT GROUPS] << 
